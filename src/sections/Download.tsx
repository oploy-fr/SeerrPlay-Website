...
const PLATFORM_ICONS = [Apple, Smartphone, Monitor];
...
export function Download() {
  ...
  const platforms = t("download.platforms", { returnObjects: true }) as PlatformItem[];
    ...
    <section id="download" className="relative overflow-hidden py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
        ...
        <h2 className="mt-3 text-3xl font-bold tracking-tight text-white sm:text-5xl">
          {t("download.titleA")} <span className="text-gradient">{t("download.titleB")}</span>
        </h2>
        ...
        <div className="mx-auto mt-10 grid max-w-3xl gap-4 sm:grid-cols-3">
          {platforms.map((p, i) => {
            ...
            const soon = i === platforms.length - 1;
            return (
              <div
                key={p.label}
                className={
              ...
              >
                ...
                <Icon className="mx-auto h-6 w-6 text-seerr-300" />
        ...
        <div className="mt-12 flex flex-col items-center justify-center gap-4 border-t border-white/10 pt-10 sm:flex-row">
          <p className="text-sm text-white/50">{t("download.discordText")}</p>
          <a
            href={DISCORD_URL}
            target="_blank"
